...
RUN cd /var/zeppelin; mvn package -Pbuild-distr -DskipTests -Pspark-2.2 -Phadoop-2.7 -Pyarn -Ppyspark -Psparkr -Pr -Pscala-2.11
...
RUN pip3 install --upgrade matplotlib seaborn jupyter grpcio mlflow
...
COPY mlflowUI /etc/init.d
RUN chmod 755 /etc/init.d/mlflowUI
